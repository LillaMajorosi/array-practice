...
/*ezt alakítjuk át alatta arrow functionné
app.get('/students', (req, res) => {//backenden azért használjuk a map-et, hogy 
	let studentlistItem = students.map(function(student){
		return {
			name: student.name,
			status: student.status,
		}
	});
	res.send(JSON.stringify(studentlistItem))
  //res.send(JSON.stringify(students)) //visszaadja a students változót. meg kell hívni a frontedbe a fetch-el az index.js-ben. 
}) */

//arrow functionné alakítás 
app.get('/students', (req, res) => {//backenden azért használjuk a map-et, hogy 
	let studentlistItem = students
		.filter(student => student.name.startsWith("P")) //ha azt szeretnénk, hogy a p betűvel kezdődőeket adja csak visssza
		.map(student => ({
			name: student.name,
			status: student.status,
		})
